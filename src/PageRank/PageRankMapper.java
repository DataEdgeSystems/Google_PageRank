...
public class PageRankMapper extends Mapper<LongWritable, Text, Text, Text> {
	...
	public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
		...
		String[] pgrnk_outListArray = pgrnk_outListString.split(Delimiters.TAB_DELIMITER);
		if (pgrnk_outListArray != null && pgrnk_outListArray.length > 0) {
			...
			context.write(new Text(current_node), new Text(OUTLINK_DELIMITER + sb.toString()));
		} else {
