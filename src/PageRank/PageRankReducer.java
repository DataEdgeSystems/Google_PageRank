	...
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		...
		for (Text pg_list : values) {
			...
			if (list.indexOf(OUTLINK_DELIMITER) == 0) {
				outlinks = list.substring(list.indexOf(OUTLINK_DELIMITER) + OUTLINK_DELIMITER.length(), list.length());
				hasOutlink = true;
			}
			else {
				String[] array = list.split(INLINK_DELIMITER);
				pgrnk = pgrnk + Double.valueOf(array[INDEX_PR]) / Double.valueOf(array[INDEX_OUTLINK]);
				hasInLink = true;
			}
		}
		if (hasOutlink) {
